...
def pt_sql(tb, rtb_list, date_data, t_type, pa_list):
    """
    ...
    if len(rtb_list) == 2:
        if 'ap' not in rtb_list:
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,sum(pt) as pt FROM %s where date = %s and bid = \'%s\' and vts = 0" \
                              % (tb, date_data, bid_val)
                    r_sql_e = "group by bid order by bid asc;"
                else:
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,sum(pt) as pt FROM %s where date = %s and bid = \'%s\' " \
                              "and %s = %s and vts = 0" \
                ...
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,substring(date, 9,10) as hour,sum(pt) as pt FROM %s where " \
                              "date = %s and bid = \'%s\' and vts = 0 " \
                              % (tb, date_data, bid_val)
                    r_sql_e = "group by bid,hour order by bid,hour  asc;"
                else:
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,substring(date, 9,10) as hour,%s,sum(pt) as pt FROM %s where " \
                              "date = %s and bid = \'%s\' and %s = %s and vts = 0 "\
                              % (rtb_list[1], tb, date_data, bid_val, rtb_list[1], any_val)
        ...
        if 'ap' not in rtb_list:
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,sum(pt) as pt FROM %s where date = %s and bid = \'%s\' " \
                              "and %s = %s and %s = %s and vts = 0"\
                ...
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,%s,substring(date, 9,10) as hour,sum(pt) as pt FROM %s where " \
                              "date = %s and bid = \'%s\' and   %s = %s and vts = 0"\
                              % (rtb_list[2], tb, date_data, bid_val, rtb_list[2], bny_val)
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,substring(date, 9,10) as hour,sum(pt) as pt FROM %s where " \
                              "date = %s and bid = \'%s\' and %s = %s and %s = %s and vts = 0" \
            ...
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,country,province,sum(pt) as pt FROM %s where date = %s " \
                              "and bid = \'%s\' and vts = 0"\
                              % (tb, date_data, bid_val)
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,country,province,sum(pt) as pt FROM %s where date = %s " \
                              "and bid = \'%s\' and  %s = %s and vts = 0"\
                              % (rtb_list[1],tb, date_data, bid_val, rtb_list[1], any_val)
                    r_sql_e = "group by bid,%s,country,province order by bid,%s,country,province asc;"\
                              % (rtb_list[1], rtb_list[1])
            else:
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,country,province,substring(date, 9,10) as hour,sum(pt) as pt " \
                              "FROM %s where date = %s and bid = \'%s\' and vts = 0"\
                              % (tb, date_data, bid_val)
                    r_sql_e = "group by bid,country,province,hour order by bid,country,province,hour asc;"
                else:
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,country,province,substring(date, 9,10) as hour,sum(pt) as pt " \
                              "FROM %s where date = %s and bid = \'%s\' and  %s = %s and vts = 0" \
                              % (rtb_list[1], tb, date_data, bid_val, rtb_list[1], any_val)
        ...
        if 'ap' not in rtb_list:
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,%s,sum(pt) as pt FROM %s where date = %s and bid = \'%s\' " \
                              "and %s = %s and %s = %s and %s = %s and vts = 0"\
                ...
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,%s,substring(date, 9,10) as hour,sum(pt) as pt FROM %s where " \
                              "date = %s and bid = \'%s\' and %s = %s and %s = %s and %s = %s and vts = 0"\
                              % (rtb_list[1], rtb_list[2], rtb_list[3], tb, date_data, bid_val,
                                 rtb_list[1], any_val, rtb_list[2], bny_val, rtb_list[3], cny_val)
            ...
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,country,province,sum(pt) as pt FROM %s where date = %s " \
                              "and bid = \'%s\' and %s = %s and %s = %s and vts = 0"\
                ...
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,%s,country,province,substring(date, 9,10) as hour,sum(pt) " \
                              "as pt FROM %s where date = %s and bid = \'%s\' and %s = %s and vts = 0" \
                              % (rtb_list[2], tb, date_data, bid_val, rtb_list[2], bny_val)
                    ...
                    r_sql_f = "SELECT bid,%s,%s,country,province,substring(date, 9,10) as hour," \
                              "sum(pt) as pt FROM %s where date = %s and bid = \'%s\' " \
                              "and %s = %s and %s = %s and vts = 0"\
                              % (rtb_list[1], rtb_list[2], tb, date_data, bid_val, rtb_list[1],
                                 any_val, rtb_list[2], bny_val)
        ...
        if 'ap' not in rtb_list:
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,%s,%s,sum(pt) as pt FROM %s where date = %s and bid = \'%s\' " \
                              "and %s = %s and %s = %s and %s = %s and %s = %s and vts = 0" \
                ...
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,%s,%s,substring(date, 9,10) as hour,sum(pt) as pt " \
            ...
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,%s,country,province,sum(pt) as pt FROM %s where date = %s " \
                              "and bid = \'%s\' and %s = %s and %s = %s and %s = %s and vts = 0" \
                              % (rtb_list[1], rtb_list[2], rtb_list[3], tb, date_data, bid_val,
                                 rtb_list[1], any_val, rtb_list[2], bny_val, rtb_list[3], cny_val)
