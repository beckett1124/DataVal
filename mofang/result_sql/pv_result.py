...
def pv_sql(tb, rtb_list, date_data, t_type, pa_list):
    """
    ...
    if len(rtb_list) == 2:
        if 'ap' not in rtb_list:
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,sum(pv) as pv FROM %s where date = %s and bid = \'%s\'" \
                              % (tb, date_data, bid_val)
                    r_sql_e = "group by bid order by bid asc;"
                else:
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,sum(pv) as pv FROM %s where date = %s and bid = \'%s\' " \
                              "and %s = %s" \
                ...
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,substring(date, 9,10) as hour,sum(pv) as pv FROM %s where " \
                              "date = %s and bid = \'%s\' " \
                              % (tb, date_data, bid_val)
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,substring(date, 9,10) as hour,%s,sum(pv) as pv FROM %s where " \
        ...
        if 'ap' not in rtb_list:
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,sum(pv) as pv FROM %s where date = %s and bid = \'%s\' " \
                              "and %s = %s and %s = %s"\
                ...
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,%s,substring(date, 9,10) as hour,sum(pv) as pv FROM %s where " \
                              "date = %s and bid = \'%s\' and   %s = %s"\
                              % (rtb_list[2], tb, date_data, bid_val, rtb_list[2], bny_val)
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,substring(date, 9,10) as hour,sum(pv) as pv FROM %s where " \
            ...
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,country,province,sum(pv) as pv FROM %s where date = %s " \
                              "and bid = \'%s\'"\
                              % (tb, date_data, bid_val)
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,country,province,sum(pv) as pv FROM %s where date = %s " \
                              "and bid = \'%s\' and  %s = %s"\
                              % (rtb_list[1],tb, date_data, bid_val, rtb_list[1], any_val)
                ...
                if rtb_list[1] == 'bid':
                    r_sql_f = "SELECT bid,country,province,substring(date, 9,10) as hour,sum(pv) as pv " \
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,country,province,substring(date, 9,10) as hour,sum(pv) as pv " \
        ...
        if 'ap' not in rtb_list:
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,%s,sum(pv) as pv FROM %s where date = %s and bid = \'%s\' " \
                ...
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,%s,substring(date, 9,10) as hour,sum(pv) as pv FROM %s where " \
            ...
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,country,province,sum(pv) as pv FROM %s where date = %s " \
                ...
                if rtb_list[1] == 'bid':
                    ...
                    r_sql_f = "SELECT bid,%s,%s,country,province,substring(date, 9,10) as hour," \
                              "sum(pv) as pv FROM %s where date = %s and bid = \'%s\' " \
                              "and %s = %s and %s = %s"\
                              % (rtb_list[1], rtb_list[2], tb, date_data, bid_val, rtb_list[1],
                                 any_val, rtb_list[2], bny_val)
        ...
        if 'ap' not in rtb_list:
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,%s,%s,sum(pv) as pv FROM %s where date = %s and bid = \'%s\' " \
                ...
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,%s,%s,substring(date, 9,10) as hour,sum(pv) as pv " \
            ...
            if t_type == 'day':
                if rtb_list[1] == 'bid':
                    ...
                    if 'fbdid' in tb:
                        rtb_list[1] = "bdid"
                    r_sql_f = "SELECT bid,%s,%s,%s,country,province,sum(pv) as pv FROM %s where date = %s " \
