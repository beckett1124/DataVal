...
def pt_sql(tb, sql, rtb_list, d_date, pa_list):
    """
    ...
    if len(rtb_list) == 3:
        if 'ap' not in rtb_list:
            if 'day' in rtb_list:
                if rtb_list[1] == 'bid':
                    f_sql_f = "select bid,sum(playtime) as pt from %s where bid = %s and year " \
                              "= \'%s\' and month = \'%s\' and day = \'%s\'"\
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,sum(playtime) as pt from %s where bid = %s and " \
                ...
                if rtb_list[1] == 'bid':
                    f_sql_f = "select bid,hour,sum(playtime) as pt from %s where bid = %s and year " \
                              "= \'%s\' and month = \'%s\' and day = \'%s\' and hour = \'%s\'"\
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,hour,sum(playtime) as pt from %s where bid = %s " \
                              "and year = \'%s\' and month = \'%s\' and day = \'%s\' and " \
                              "hour = \'%s\'and %s = %s"\
                              % (rtb_list[1], fact_table, bid_val, str(d_date[1:5]), str(d_date[5:7]),
        ...
        if 'ap' not in rtb_list:
            if 'day' in rtb_list:
                if rtb_list[1] == 'bid':
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,%s,sum(playtime) as pt from %s where bid = %s " \
                              "and year = \'%s\' and month = \'%s\' and day = \'%s\' " \
                              "and %s = %s and %s = %s" \
                              % (rtb_list[1], rtb_list[2], fact_table, bid_val, str(d_date[1:5]),
                                 str(d_date[5:7]), str(d_date[7:-1]), rtb_list[1], any_val,
                                 rtb_list[2], bny_val)
                ...
                if rtb_list[1] == 'bid':
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,%s,hour,sum(playtime) as pt from %s where bid = %s and " \
                              "year = \'%s\' and month = \'%s\' and day = \'%s\' and hour = \'%s\' " \
                              "and %s = %s and %s = %s" \
                              % (rtb_list[1], rtb_list[2], fact_table, bid_val, str(d_date[1:5]),
                                 str(d_date[5:7]), str(d_date[7:9]), str(d_date[9:-1]),
            ...
            if 'day' in rtb_list:
                if rtb_list[1] == 'bid':
                    f_sql_f = "select bid,country,province,sum(playtime) as pt from %s where bid = %s " \
                              "and year = \'%s\' and month = \'%s\' and day = \'%s\'"\
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,country,province,sum(playtime) as pt from %s where " \
                ...
                if rtb_list[1] == 'bid':
                    f_sql_f = "select bid,country,province,hour,sum(playtime) as pt from %s " \
                              "where bid = %s and year = \'%s\' and month = \'%s\' and day " \
                              "= \'%s\' and hour = \'%s\'"\
                              % (fact_table, bid_val, str(d_date[1:5]), str(d_date[5:7]),
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,country,province,hour,sum(playtime) as pt from %s where " \
                              "bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and " \
                              "hour = \'%s\' and %s = %s" \
                              % (rtb_list[1], fact_table, bid_val, str(d_date[1:5]), str(d_date[5:7]),
                                 str(d_date[7:9]), str(d_date[9:-1]), rtb_list[1], any_val)
        ...
        if 'ap' not in rtb_list:
            if 'day' in rtb_list:
                if rtb_list[1] == 'bid':
                    f_sql_f = "select bid,%s,%s,sum(playtime) as pt from %s where bid = %s and " \
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,%s,%s,sum(playtime) as pt from %s where bid = %s and " \
                              "year = \'%s\' and month = \'%s\' and day = \'%s\' and %s = %s and %s = %s" \
                              " and %s = %s" \
                ...
                if rtb_list[1] == 'bid':
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,%s,%s,hour,sum(playtime) as pt from %s where bid = %s and " \
            ...
            if 'day' in rtb_list:
                if rtb_list[1] == 'bid':
                    f_sql_f = "select bid,%s,country,province,sum(playtime) as pt from %s where " \
                              "bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and %s = %s"\
                              % (rtb_list[2], fact_table, bid_val, str(d_date[1:5]), str(d_date[5:7]),
                                 str(d_date[7:-1]), rtb_list[2], bny_val)
                    f_sql_e = "group by bid,%s,country,province order by bid,%s,country,province asc;"\
                              % (rtb_list[2], rtb_list[2])
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,%s,country,province,sum(playtime) as pt from %s where " \
                              "bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' " \
                              "and %s = %s and %s = %s" \
                              % (rtb_list[1], rtb_list[2], fact_table, bid_val, str(d_date[1:5]), str(d_date[5:7]),
                                 str(d_date[7:-1]), rtb_list[1], any_val, rtb_list[2], bny_val)
                ...
                if rtb_list[1] == 'bid':
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,%s,country,province,hour,sum(playtime) as pt from %s " \
                              "where bid = %s and year = \'%s\' and month = \'%s\' and day " \
                              "= \'%s\' and hour = \'%s\' and %s = %s and %s = %s" \
                              % (rtb_list[1], rtb_list[2], fact_table, bid_val, str(d_date[1:5]), str(d_date[5:7]),
        ...
        if 'ap' not in rtb_list:
            if 'day' in rtb_list:
                if rtb_list[1] == 'bid':
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,%s,%s,%s,sum(playtime) as pt from %s where bid = %s and " \
                              "year = \'%s\' and month = \'%s\' and day = \'%s\' and %s = %s and %s = %s" \
                              " and %s = %s and %s = %s" \
                ...
                if rtb_list[1] == 'bid':
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,%s,%s,%s,hour,sum(playtime) as pt from %s where bid = %s and " \
                              "year = \'%s\' and month = \'%s\' and day = \'%s\' and hour = \'%s\' and %s " \
            ...
            if 'day' in rtb_list:
                if rtb_list[1] == 'bid':
                    f_sql_f = "select bid,%s,%s,country,province,sum(playtime) as pt from %s where " \
                              "bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and %s " \
                              "= %s and %s = %s"\
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,%s,%s,country,province,sum(playtime) as pt from %s where " \
                              "bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' " \
                              "and %s = %s and %s = %s and %s = %s" \
                              % (rtb_list[1], rtb_list[2], rtb_list[3], fact_table, bid_val,
                                 str(d_date[1:5]), str(d_date[5:7]), str(d_date[7:-1]),
                                 rtb_list[1], any_val, rtb_list[2], bny_val, rtb_list[3], cny_val)
                    f_sql_e = "group by bid,%s,%s,%s,country,province order by bid,%s,%s,%s," \
                              "country,province asc;"\
                              % (rtb_list[1], rtb_list[2], rtb_list[3], rtb_list[1], rtb_list[2], rtb_list[3])
            else:
                if rtb_list[1] == 'bid':
                    f_sql_f = "select bid,%s,%s,country,province,hour,sum(playtime) as pt from %s where " \
                              "bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' " \
                    ...
                    if 'sid' in tb:
                        ...
                        rtb_list[1] = "bdid"
                    f_sql_f = "select bid,%s,%s,%s,country,province,hour,sum(playtime) as pt from %s " \
                              "where bid = %s and year = \'%s\' and month = \'%s\' and day " \
                              "= \'%s\' and hour = \'%s\' and %s = %s and %s = %s and %s = %s" \
                              % (rtb_list[1], rtb_list[2], rtb_list[3], fact_table, bid_val,
                                 str(d_date[1:5]), str(d_date[5:7]), str(d_date[7:9]),
                                 str(d_date[9:-1]), rtb_list[1], any_val, rtb_list[2],
                                 bny_val, rtb_list[3], cny_val)
                    f_sql_e = "group by bid,hour,%s,%s,%s,country,province order by bid,hour,%s,%s,%s,country," \
                              "province asc;"\
                              % (rtb_list[1], rtb_list[2], rtb_list[3], rtb_list[1], rtb_list[2], rtb_list[3])
