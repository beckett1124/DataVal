...
data_list = ['pid', 'sid', 'cid', 'bdid', 'isp', 'chtype', 'ch', 'duration', 'playtype', 'isfull']
...
def select_mode(ip, port, user, passwd, dbname, sql, tb):
    ...
    cur = conn.cursor()
    tn = tb + '_' + dbname + '.csv'
    tn = os.path.join('/home/qibin/pt/isp/data', tn)
    f = open(tn, 'w')
    w = csv.writer(f)
    cur.execute(sql)
    while True:
...
def get_tname(file, db1, p, t):
    ...
    if p == 'uv':
        if t == "hour":
            tb_sql = "select tablename from pg_tables where tablename like 'uv%isp%hour';"
        else:
            tb_sql = "select tablename from pg_tables where tablename like 'uv%isp%day';"
    elif p == 'vv':
        if t == "hour":
            tb_sql = "select tablename from pg_tables where tablename like 'vv%isp%hour';"
        else:
            tb_sql = "select tablename from pg_tables where tablename like 'vv%isp%day';"
    elif p == 'pt':
        if t == "hour":
            tb_sql = "select tablename from pg_tables where tablename like 'pt%isp%hour';"
        else:
            tb_sql = "select tablename from pg_tables where tablename like 'pt%isp%day';"
    elif p == 'dau':
        if t == "hour":
            tb_sql = "select tablename from pg_tables where tablename like 'dau%isp%hour';"
        else:
            tb_sql = "select tablename from pg_tables where tablename like 'dau%isp%day';"
...
def date_d(ip, port, user, password, dbname, c_type):
    ...
    d_sql = ""
    if c_type == 'hour':
        d_sql = "select to_char(now() - interval '50hours','YYYYMMDDhh24');"
    elif c_type == 'day':
        d_sql = "select to_char(now() - interval '50hours','YYYYMMDD');"
...
def data_uv_sql(tb, rtb_list, date_data, t_type, d_type, pid_list, cid_list, sid_list, bdid_list, vid_list):
    ...
    if rtb_list[0] == 'cid':
        if len(rtb_list) == 1:
            r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and cid = \'%s\' group by " \
                    "isp, %s order by isp, %s desc;" \
                    % (d_type, tb, date_data, bid_val, str(choice(cid_list)), d_type, d_type)
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                        "chtype = \'%s\' group by isp, %s order by isp, %s desc;" \
                        % (d_type, tb, date_data, bid_val, str(choice(cid_list)), str(choice(chtype_list)), d_type, d_type)
            elif rtb_list[1] == 'playtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                    "playtype = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
                                                                         str(choice(cid_list)), str(choice(playtype_list)), d_type, d_type)
            elif rtb_list[1] == 'isfull':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                    "isfull =\'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
                                                                       str(choice(cid_list)),
                                                                       str(choice(isfull_list)), d_type, d_type)
            elif rtb_list[1] == 'duration':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                    "duration =\'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
                                                                       str(choice(cid_list)),
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'playtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                        "chtype = \'%s\' and playtype = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data,
                                                                                                     bid_val,
                                                                                             str(choice(cid_list)),
                ...
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                        "chtype = \'%s\' and isfull = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data,
                                                                                                   bid_val,
                                                                                           str(choice(cid_list)),
                                                                                           str(choice(chtype_list)),
                                                                                           str(choice(isfull_list)), d_type, d_type)
                    ...
                    r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and cid = \'%s\'and " \
                            "chtype = \'%s\' and duration = \'%s\'group by isp, %s order by isp, %s desc;" % (d_type, tb,
                                                                                                        date_data,
                                                                                                 bid_val,
                                                                                                 str(choice(cid_list)),
                                                                                                 str(choice(chtype_list)),
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'isfull' and rtb_list[3] == 'playtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                        "chtype = \'%s\' and isfull = \'%s\' and playtype = \'%s\' group by isp, %s order by " \
                        "isp, %s desc;" \
                ...
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                        "chtype = \'%s\' and duration = \'%s\' and playtype = \'%s\' group by isp, %s order by isp " \
        ...
        if len(rtb_list) == 1:
            r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and pid = \'%s\' group by " \
                    "isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val, str(choice(pid_list)), d_type, d_type)
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and pid = \'%s\' and " \
                        "chtype = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
                ...
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and pid = \'%s\' and " \
                        "isfull = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, str(choice(bid_list)),
                                                                           str(choice(pid_list)), str(choice(isfull_list)), d_type, d_type)
            elif rtb_list[1] == 'duration':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and pid = \'%s\' and " \
                        "duration = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, str(choice(bid_list)),
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'playtype':
                ...
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and pid = \'%s\' and " \
                        "chtype = \'%s\' and isfull = \'%s\' group by isp, %s order by isp, %s desc;" \
                        % (d_type, tb, date_data, bid_val, str(choice(pid_list)), str(choice(chtype_list)),
                           str(choice(isfull_list)), d_type, d_type)
            elif rtb_list[1] == 'chtype' and rtb_list[2] == 'duration':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and pid = \'%s\' and " \
                        "chtype = \'%s\' and duration = \'%s\' group by isp, %s order by isp, %s desc;" \
                        % (d_type, tb, date_data, bid_val, str(choice(pid_list)), str(choice(chtype_list)),
                           str(choice(duration_list)), d_type, d_type)
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'isfull' and rtb_list[3] == 'playtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and pid = \'%s\' and " \
                        "chtype = \'%s\' and isfull = \'%s\' and playtype = \'%s\' group by isp, %s order by isp, %s desc;" \
                        % (d_type, tb, date_data, bid_val, str(choice(pid_list)), str(choice(chtype_list)),
                           str(choice(isfull_list)), str(choice(playtype_list)), d_type, d_type)
            elif rtb_list[1] == 'chtype' and rtb_list[2] == 'duration' and rtb_list[3] == 'playtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\'and pid = \'%s\' and " \
                        "chtype = \'%s\' and duration = \'%s\' and playtype = \'%s\' group by isp, %s order by " \
                        "isp, %s desc; " \
                        % (d_type, tb, date_data, bid_val, str(choice(pid_list)),str(choice(chtype_list)),
                           str(choice(duration_list)), str(choice(playtype_list)), d_type, d_type)
            else:
        ...
        if len(rtb_list) == 1:
            r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and sid = \'%s\' group by " \
                    "isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val, str(choice(sid_list)), d_type, d_type)
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and sid = \'%s\' and " \
                        "chtype = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
                                                                           str(choice(sid_list)),
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'playtype':
                ...
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and sid = \'%s\' and " \
                        "chtype = \'%s\' and duration = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data,
                                                                                                     bid_val,
                                                                                             str(choice(sid_list)),
                                                                                             str(choice(chtype_list)),
                                                                                                 str(choice(
        ...
        if len(rtb_list) == 1:
            r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and bdid = \'%s\' group by " \
                    "isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val, str(choice(bdid_list)),
                                                          d_type, d_type)
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and bdid = \'%s\' and " \
                        "chtype = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'isfull' and rtb_list[3] == 'playtype':
                ...
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and bdid = \'%s\' and " \
                        "chtype = \'%s\' and duration = \'%s\' and playtype = \'%s\' group by isp, %s order by isp, %s desc;" \
                        % (d_type, tb, date_data, bid_val, str(choice(bdid_list)), str(choice(chtype_list)),
                                   str(choice(duration_list)), str(choice(playtype_list)), d_type, d_type)
            else:
                raise Exception("fbdid has no such table!")
        ...
        if len(rtb_list) == 1:
            r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' group by isp, %s order by " \
                    "isp, %s desc;" % (d_type, tb, date_data, bid_val, d_type, d_type)
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'playtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and playtype = \'%s\' " \
                        "group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
                                                               str(choice(playtype_list)), d_type, d_type)
            elif rtb_list[1] == 'chtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and chtype = \'%s\' " \
                        "group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
                                                               str(choice(playtype_list)), d_type, d_type)
            elif rtb_list[1] == 'isfull':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and isfull = \'%s\' group " \
                        "by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
                                                                           str(choice(isfull_list)), d_type, d_type)
            elif rtb_list[1] == 'duration':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and " \
                        "duration = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
                                                                           str(choice(duration_list)), d_type, d_type)
            else:
            ...
            if rtb_list[1] == 'playtype' and rtb_list[2] == 'isfull':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and isfull = \'%s\' and playtype = \'%s\' " \
                        "group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val, choice(isfull_list),
                                                               str(choice(playtype_list)), d_type, d_type)
                ...
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and duration = \'%s\' and isfull = \'%s\' " \
                        "group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val, choice(duration_list),
                                                               str(choice(isfull_list)), d_type, d_type)
            elif rtb_list[1] == 'playtype' and rtb_list[2] == 'duration':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and duration = \'%s\' and playtype = \'%s\' " \
                        "group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val, choice(duration_list),
                                                               str(choice(playtype_list)), d_type, d_type)
    elif rtb_list[0] == 'fbdid':
        if len(rtb_list) == 1:
            r_sql = "select isp, %s from %s where date = %s and bid = %s and bdid = %s group by " \
                    "isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val, str(choice(bdid_list)), d_type, d_type)
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and bdid = \'%s\'and " \
                        "chtype = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'playtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and bdid = \'%s\' and " \
                        "chtype = \'%s\' and playtype = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb,
                                                                                             date_data,
                                                                                                     bid_val,
                                                                                             str(choice(sid_list)),
                                                                                             str(choice(chtype_list)),
        ...
        if len(rtb_list) == 1:
            r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and vid = \'%s\' group by " \
                    "isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val, str(choice(vid_list)), d_type, d_type)
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and vid = \'%s\' and " \
                        "chtype = \'%s\' group by isp, %s order by isp, %s desc;" % (d_type, tb, date_data, bid_val,
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'isfull' and rtb_list[3] == 'playtype':
                ...
                r_sql = "select isp, %s from %s where date = %s and bid = \'%s\' and vid = \'%s\' and " \
                        "chtype = \'%s\' and duration = \'%s\' and playtype = \'%s\' group by isp, %s  order by isp, %s  " \
...
def data_r_sql(tb, rtb_list, date_data, t_type, d_type, pid_list, cid_list, sid_list, bdid_list, vid_list):
    ...
    if rtb_list[0] == 'cid':
        if len(rtb_list) == 1:
            r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and cid = \'%s\' group by " \
                    "isp order by isp desc;" % (d_type, tb, date_data, bid_val, str(choice(cid_list)))
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                        "chtype = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
                                                                               str(choice(cid_list)), str(choice(chtype_list)))
            elif rtb_list[1] == 'playtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                    "playtype = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
                                                                         str(choice(cid_list)), str(choice(playtype_list)))
            elif rtb_list[1] == 'isfull':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                    "isfull =\'%s\' group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
                                                                       str(choice(cid_list)),
                                                                       str(choice(isfull_list)))
            elif rtb_list[1] == 'duration':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                    "duration =\'%s\' group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
                                                                       str(choice(cid_list)),
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'playtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                        "chtype = \'%s\' and playtype = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data,
                                                                                                     bid_val,
                                                                                             str(choice(cid_list)),
                                                                                             str(choice(chtype_list)),
                                                                                             str(choice(
                ...
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                        "chtype = \'%s\' and isfull = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data,
                                                                                                   bid_val,
                                                                                           str(choice(cid_list)),
                                                                                           str(choice(chtype_list)),
                                                                                           str(choice(isfull_list)))
                    ...
                    r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and cid = \'%s\'and " \
                            "chtype = \'%s\' and duration = \'%s\'group by isp order by isp desc;" % (d_type, tb,
                                                                                                        date_data,
                                                                                                 bid_val,
                                                                                                 str(choice(cid_list)),
                                                                                                 str(choice(chtype_list)),
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'isfull' and rtb_list[3] == 'playtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                        "chtype = \'%s\' and isfull = \'%s\' and playtype = \'%s\' group by isp order by isp desc;" \
                        % (d_type, tb, date_data, bid_val, str(choice(cid_list)), str(choice(chtype_list)),
                           str(choice(isfull_list)), str(choice(playtype_list)))
            elif rtb_list[1] == 'chtype' and rtb_list[2] == 'duration' and rtb_list[3] == 'playtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and cid = \'%s\' and " \
                        "chtype = \'%s\' and duration = \'%s\' and playtype = \'%s\' group by isp order by isp " \
        ...
        if len(rtb_list) == 1:
            r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and pid = \'%s\' group by " \
                    "isp order by isp desc;" % (d_type, tb, date_data, bid_val, str(choice(pid_list)))
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and pid = \'%s\' and " \
                        "chtype = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
                ...
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and pid = \'%s\' and " \
                        "isfull = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data, str(choice(bid_list)),
                                                                           str(choice(pid_list)), str(choice(isfull_list)))
            elif rtb_list[1] == 'duration':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and pid = \'%s\' and " \
                        "duration = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data, str(choice(bid_list)),
        ...
        if len(rtb_list) == 1:
            r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and sid = \'%s\' group by " \
                    "isp order by isp desc;" % (d_type, tb, date_data, bid_val, str(choice(sid_list)))
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and sid = \'%s\' and " \
                        "chtype = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'playtype':
                ...
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and sid = \'%s\' and " \
                        "chtype = \'%s\' and duration = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data,
                                                                                                     bid_val,
                                                                                             str(choice(sid_list)),
                                                                                             str(choice(chtype_list)),
                                                                                                 str(choice(
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'isfull' and rtb_list[3] == 'playtype':
                ...
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and sid = \'%s\' and " \
                        "chtype = \'%s\' and duration = \'%s\' and playtype = \'%s\' group by isp order by isp desc;" \
                        % (d_type, tb, date_data, bid_val, str(choice(sid_list)), str(choice(chtype_list)),
                           str(choice(duration_list)), str(choice(playtype_list)))
            else:
                raise Exception("sid has no such table!")
        ...
        if len(rtb_list) == 1:
            r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and bdid = \'%s\' group by " \
                    "isp order by isp desc;" % (d_type, tb, date_data, bid_val, str(choice(bdid_list)))
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and bdid = \'%s\' and " \
                        "chtype = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
                                                                           str(choice(bdid_list)),
                                                                           str(choice(chtype_list)))
            elif rtb_list[1] == 'playtype':
        ...
        if len(rtb_list) == 1:
            r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' group by isp order by " \
                    "isp desc;" % (d_type, tb, date_data, bid_val)
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'playtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and playtype = \'%s\' " \
                        "group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
                                                               str(choice(playtype_list)))
            elif rtb_list[1] == 'chtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and chtype = \'%s\' " \
                        "group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
                                                               str(choice(chtype_list)))
            elif rtb_list[1] == 'duration':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and duration = \'%s\' " \
                        "group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
                                                               str(choice(duration_list)))
            elif rtb_list[1] == 'isfull':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and isfull = \'%s\' " \
                        "group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
                                                               str(choice(isfull_list)))
            else:
                raise Exception("bid has no such table!")
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'playtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and " \
                        "chtype = \'%s\' and playtype = \'%s\' group by isp order by isp desc;" \
                        % (d_type, tb, date_data, bid_val, str(choice(chtype_list)), str(choice(playtype_list)))
            elif rtb_list[1] == 'chtype' and rtb_list[2] == 'isfull':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and " \
                        "chtype = \'%s\' and isfull = \'%s\' group by isp order by isp desc;" \
                        % (d_type, tb, date_data, bid_val, str(choice(chtype_list)), str(choice(isfull_list)))
            elif rtb_list[1] == 'isfull' and rtb_list[2] == 'chtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and " \
                        "isfull = \'%s\' and chtype = \'%s\' group by isp order by isp desc;" \
                        % (d_type, tb, date_data, bid_val, str(choice(isfull_list)), str(choice(chtype_list)))
            elif rtb_list[1] == 'chtype' and rtb_list[2] == 'duration':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and " \
                        "chtype = \'%s\' and duration = \'%s\' group by isp order by isp desc;" \
            ...
            if rtb_list[1] == 'playtype' and rtb_list[2] == 'duration':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and " \
                        "playtype = \'%s\' and duration = \'%s\' group by isp order by isp desc;" \
        ...
        if len(rtb_list) == 1:
            ...
            if rtb_list[1] == 'chtype' and rtb_list[2] == 'playtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and bdid = \'%s\' and " \
                        "chtype = \'%s\' and playtype = \'%s\' group by isp order by isp desc;" % (d_type, tb,
                                                                                             date_data,
                                                                                                     bid_val,
                                                                                             str(choice(bdid_list)),
                                                                                             str(choice(chtype_list)),
        ...
        if len(rtb_list) == 1:
            r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and vid = \'%s\' group by " \
                    "isp order by isp desc;" % (d_type, tb, date_data, bid_val, str(choice(vid_list)))
        elif len(rtb_list) == 2:
            if rtb_list[1] == 'chtype':
                r_sql = "select isp, sum(%s) from %s where date = %s and bid = \'%s\' and vid = \'%s\' and " \
                        "chtype = \'%s\' group by isp order by isp desc;" % (d_type, tb, date_data, bid_val,
...
def data_f_sql(tb, sql, d_date, s_first):
    ...
    if 'cid' in tb:
        if and_count == 2:
            ...
            if 'hour' in tb:
                f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour = \'%s\' " \
                        "and cid = %s group by isp order by isp desc;" \
                ...
                f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and cid = %s " \
                        "group by isp order by isp desc;" \
            ...
            if 'chtype' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and " \
                            "hour = \'%s\' and cid = %s and chtype = %s group by isp order by isp desc;" \
                            % (bid_val, str(d_date[1:5]), str(d_date[5:7]), str(d_date[7:9]), str(d_date[9:-1]),
                               cid_val, chtype_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and " \
                            "cid = %s and chtype = %s group by isp order by isp desc;" \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour " \
                            "= \'%s\' and cid = %s and playtype = %s group by isp order by isp desc;" \
                            % (bid_val, str(d_date[1:5]), str(d_date[5:7]), str(d_date[7:9]), str(d_date[9:-1]),
                               cid_val, playtype_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and cid = %s " \
                            "and playtype = %s group by isp order by isp desc;" \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour " \
                            "= \'%s\' and cid = %s and isfull = %s group by isp order by isp desc;" \
                            % (bid_val, str(d_date[1:5]), str(d_date[5:7]),
                    ...
                    f_sql = "where bid = %s and year = \'%s\' " \
                            ...
                            "by isp order by isp desc;" % (bid_val, str(d_date[1:5]), str(d_date[5:7]),
                                                             str(d_date[7:-1]), cid_val, isfull_val)
            elif 'duration' in tb:
                duration_val = sql.split("and")[3].split(";")[0].split("duration")[1].split('group')[0].split("=")[1]
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour " \
                            "= \'%s\' and cid = %s and vts = %s group by isp order by isp desc;" \
                            % (bid_val, str(d_date[1:5]), str(d_date[5:7]),
            ...
            if 'chtype' in tb and 'playtype' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and " \
                            ...
                            "isp order by isp desc;" % (bid_val, str(d_date[1:5]), str(d_date[5:7]),
                                                                      str(d_date[7:9]), str(d_date[9:-1]), cid_val,
                                                                      chtype_val, playtype_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and cid = %s and chtype = %s and playtype = " \
                            "%s group by isp order by isp desc;" % (bid_val, str(d_date[1:5]), str(d_date[5:7]),
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour " \
                            ...
                            "and isfull = %s group by isp order by isp desc;" \
                            % (bid_val, str(d_date[1:5]), str(d_date[5:7]), str(d_date[7:9]), str(d_date[9:-1]),
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            ...
                            "vts = %s group by isp order by isp desc;" \
                            % (bid_val, str(d_date[1:5]), str(d_date[5:7]),
                                                                      str(d_date[7:9]), str(d_date[9:-1]), cid_val,
            ...
            if 'chtype' in tb and 'isfull' in tb and 'playtype' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour " \
                            ...
                            "%s and playtype = %s group by isp order by isp desc;" \
                            % (bid_val, str(d_date[1:5]), str(d_date[5:7]), str(d_date[7:9]), str(d_date[9:-1]),
                               cid_val, chtype_val, isfull_val, playtype_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and cid = %s and chtype = %s and isfull = " \
                            "%s and playtype = %s group by isp order by isp desc;" % (bid_val, str(d_date[1:5]),
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            ...
                            "= %s and vts = %s and playtype = %s group by isp order by isp desc;" \
                            % (bid_val, str(d_date[1:5]), str(d_date[5:7]), str(d_date[7:9]), str(d_date[9:-1]), cid_val,
        ...
        if and_count == 2:
            ...
            if 'hour' in tb:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and pid = %s group by isp order by isp desc;" \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], pid_val)
            else:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and pid = %s group by isp order by isp desc;" \
            ...
            if 'chtype' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            ...
                            "= %s group by isp order by isp desc;" \
                    ...
                    f_sql = "where bid = %s and year = \'%s\' " \
                            ...
                            "isp order by isp desc;" % (bid_val, d_date[1:5], d_date[5:7], d_date[7:-1],
                                                          pid_val, chtype_val)
            elif 'playtype' in tb:
                playtype_val = sql.split("and")[3].split(";")[0].split("playtype")[1].split('group')[0].split("=")[1]
                if 'hour' in tb:
            ...
            if 'chtype' in tb and 'playtype' in tb:
                ...
                if 'hour' in tb:
                    ...
                    f_sql = "where bid = %s and year = \'%s\' " \
                            ...
                            "%s group by isp order by isp desc;" % (bid_val, d_date[1:5], d_date[5:7],
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and hour = \'%s\' and pid = %s and chtype " \
                            "= %s and isfull = %s group by isp order by isp desc;" \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and hour = \'%s\' and pid = %s and chtype " \
                            "= %s and vts = %s group by isp order by isp desc;" \
                            % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], pid_val, chtype_val,
                                                                      duration_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' " \
            ...
            if 'chtype' in tb and 'isfull' in tb and 'playtype' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and hour = \'%s\' and pid = %s and chtype " \
                            "= %s and isfull = %s and playtype = %s group by isp order by isp desc;" \
                            % (bid_val, d_date[1:5],
                ...
                if 'hour' in tb:
                    ...
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and pid = %s and chtype = %s and vts = " \
                            "%s and playtype = %s group by isp order by isp desc;" % (bid_val, d_date[1:5],
                                                                                        d_date[5:7], d_date[7:-1],
                                                                                        pid_val, chtype_val,
                                                                                        duration_val,
                                                                                        playtype_val)
        ...
        if and_count == 2:
            ...
            if 'hour' in tb:
                f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour = \'%s\' " \
                        "and seriesid = %s group by isp order by isp desc;" \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], sid_val)
            else:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and seriesid = %s group by isp order by isp desc;" \
            ...
            if 'chtype' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour " \
                            "= \'%s\' and seriesid = %s and chtype = %s group by isp order by isp desc;" \
                            % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9],
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour " \
                            ...
                            "group by isp order by isp desc;" % (bid_val, d_date[1:5], d_date[5:7],
                                                                   d_date[7:9], d_date[9:-1], sid_val, isfull_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and seriesid = %s and isfull = %s group by isp " \
                            "order by isp desc;" % (bid_val, d_date[1:5], d_date[5:7], d_date[7:-1],
                                                     sid_val, isfull_val)
            elif 'duration' in tb:
                duration_val = sql.split("and")[3].split(";")[0].split("duration")[1].split('group')[0].split("=")[1]
                if 'hour' in tb:
                    ...
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and seriesid = %s and vts = %s group by isp " \
                            "order by isp desc;" % (bid_val, d_date[1:5], d_date[5:7], d_date[7:-1],
            ...
            if 'chtype' in tb and 'playtype' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\'" \
                            ...
                            "and chtype = %s and playtype = %s group by isp order by isp desc;"\
                            % (bid_val, d_date[1:5], d_date[5:7],
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            ...
                            "chtype = %s and vts = %s group by isp order by isp desc;" \
                            % (bid_val, d_date[1:5], d_date[5:7],
                                                                      d_date[7:9], d_date[9:-1], sid_val,
                                                                      chtype_val, duration_val)
                else:
        ...
        if and_count == 2:
            ...
            if 'hour' in tb:
                f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour " \
                        "= \'%s\' and bdid = %s group by isp order by isp desc;" \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], bdid_val)
            else:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and bdid = %s group by isp order by isp " \
            ...
            if 'chtype' in tb:
                ...
                if 'hour' in tb:
                    ...
                    f_sql = "where bid = %s and year = \'%s\' " \
                            ...
                            "by isp order by isp desc;" % (bid_val, d_date[1:5], d_date[5:7],
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            ...
                            "playtype = %s group by isp order by isp desc;" \
                            % (bid_val, d_date[1:5], d_date[5:7],
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and hour = \'%s\' and bdid = %s and " \
                            "isfull = %s group by isp order by isp desc;" \
                            % (bid_val, d_date[1:5], d_date[5:7],
                                                             d_date[7:9], d_date[9:-1], bdid_val,
                                                             isfull_val)
                else:
            ...
            if 'chtype' in tb and 'playtype' in tb:
                ...
                if 'hour' in tb:
                    ...
                    f_sql = "where bid = %s and year = \'%s\' " \
                            ...
                            "playtype = %s group by isp order by isp desc;" % (bid_val, d_date[1:5],
                                                                                 d_date[5:7], d_date[7:-1],
                                                                                 bdid_val, chtype_val,
                                                                                 playtype_val)
            elif 'chtype' in tb and 'isfull' in tb:
        ...
        if and_count == 1:
            ...
            if 'hour' in tb:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' group by isp order by isp desc;" % (bid_val_r,
                                                                                                     d_date[1:5],
                                                                                                     d_date[5:7],
                                                                                                     d_date[7:9],
                                                                                                     d_date[9:-1])
                ...
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' group by isp order by isp desc;" % (bid_val_r,
            ...
            if 'playtype' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and playtype = %s group by isp order " \
                        "by isp desc; " \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], playtype_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and playtype = %s group by isp order by isp " \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and chtype = %s group by isp order " \
                        "by isp desc; " \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], chtype_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and chtype = %s group by isp order by isp " \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and isfull = %s group by isp order " \
                        "by isp desc; " \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], isfull_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and isfull = %s group by isp order by isp " \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and vts = %s group by isp order " \
                        "by isp desc; " \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], duration_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and vts = %s group by isp order by isp " \
        ...
        if and_count == 2:
            ...
            if 'hour' in tb:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and bdid = %s group by isp order " \
                        "by isp desc;" \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], bdid_val)
            else:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and bdid = %s group by isp order by isp desc;" \
            ...
            if 'chtype' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and hour = \'%s\' and bdid = %s and chtype " \
                            "= %s' group by " \
                            "isp order by isp desc;" % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9],
                                                          d_date[9:-1], bdid_val, chtype_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' " \
        ...
        if and_count == 2:
            ...
            if 'hour' in tb:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and pid = %s group by isp order " \
                        "by isp desc;" \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], vid_val)
            else:
                f_sql = "where bid = %s and year = %s and " \
                        "month = %s and day = %s and vid = %s group by isp order by isp desc;" \
...
def data_pt_sql(tb, sql, d_date, s_first):
    ...
    if 'cid' in tb:
        ...
        if and_count == 2:
            ...
            if 'hour' in tb:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and pid = %s group by isp " \
                        "order by isp desc;" \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], pid_val)
            else:
                f_sql = "where bid = %s and year = \'%s\' and " \
        ...
        if and_count == 2:
            ...
            if 'chtype' in tb:
                ...
                if 'hour' in tb:
                    ...
                    f_sql = "where bid = %s and year = \'%s\' " \
                            "and month = \'%s\' and day = \'%s\' and seriesid = %s and vts = %s group by isp, " \
                            "playtime order by isp, playtime desc;" % (bid_val, d_date[1:5], d_date[5:7], d_date[7:-1],
                                                     sid_val, duration_val)
        elif and_count == 4:
            sid_val = sql.split("and")[2].split("sid")[1].split("=")[1]
            if 'chtype' in tb and 'playtype' in tb:
        ...
        if and_count == 1:
            ...
            if 'hour' in tb:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' group by isp order by isp " \
            ...
            if 'playtype' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and playtype = %s group by isp " \
                        "order by isp desc; " \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], playtype_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and chtype = %s group by isp " \
                        "order by isp desc; " \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], chtype_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and isfull = %s group by isp " \
                        "order by isp desc; " \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], isfull_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and vts = %s group by isp " \
                        "order by isp desc; " \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], duration_val)
                else:
                    f_sql = "where bid = %s and year = \'%s\' and " \
            ...
            if 'playtype' in tb and 'isfull' in tb:
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and hour = \'%s\' " \
                            "and playtype = %s  and isfull = %s group by isp order by isp desc; " \
                    ...
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and playtype = %s " \
                            "and isfull_val = %s group by isp order by isp desc;" \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and " \
                            "hour = \'%s\' and chtype = %s and isfull = %s group by isp order by isp desc; " \
                ...
                if 'hour' in tb:
                    f_sql = "where bid = %s and year = \'%s\' and month = \'%s\' and day = \'%s\' and " \
                            "hour = \'%s\' and duration = %s and isfull = %s group by isp order by isp desc; " \
        ...
        if and_count == 2:
            ...
            if 'hour' in tb:
                f_sql = "where bid = %s and year = \'%s\' and " \
                        "month = \'%s\' and day = \'%s\' and hour = \'%s\' and bdid = %s group by isp " \
                        "order by isp desc;" \
                        % (bid_val, d_date[1:5], d_date[5:7], d_date[7:9], d_date[9:-1], bdid_val)
            else:
                f_sql = "where bid = %s and year = \'%s\' and " \
...
def fact_data(ip2, port2, user2, password2, dbname2, tb, sql, d_date):
    ...
    if 'ap' in tb:
        return
    if 'uv' in tb:
        sql_first = "select isp,count(distinct did) as uv from vv_fact"
    elif 'vv' in tb:
        sql_first = "select isp,count(1) as vv from vv_fact"
    elif 'dau' in tb:
        sql_first = "select isp,count(istinct did) as dau from pv_fact"
    elif 'pt' in tb:
        sql_first = "select isp, sum(playtime) as playtime from playtime_fact"
...
def main(file, re_db, db2):
    logger.init_log('./log/isp', level=logging.DEBUG)
    pamram_list = ['pt']
    time_list = ['hour', 'day']
    ip2, port2, user2, password2, dbname2 = parse_conf(file, db2)
    ip1, port1, user1, password1, dbname1 = parse_conf(file, re_db)
    ...
    for pa in pamram_list:
        for time in time_list:
            ...
            for i in range(len(re_tb)):
                ...
                print tb
                if 'ap' in tb:
                    logging.info("Result Table info no need to select: %s" % tb)
                    print("Result Table info no need to select: %s" % tb)
                else:
                    rtb_list, rtb_type = result_dtable(tb)
                    if rtb_type == 'hour':
                        d_date = date_d(ip1, port1, user1, password1, dbname1, 'hour')
                    ...
                    ff1 = os.path.join('/home/qibin/pt/isp/data', r_data)
                    ff2 = os.path.join('/home/qibin/pt/isp/data', f_data)
                    rf1 = os.path.join('/home/qibin/pt/isp/data', 'bak' + '_' + r_data.split('/')[-1])
                    rf2 = os.path.join('/home/qibin/pt/isp/data', 'bak' + '_' + f_data.split('/')[-1])
